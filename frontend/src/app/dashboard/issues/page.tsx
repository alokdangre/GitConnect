  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-slate-100">Issues</h2>
      ...
      {issue.labels && issue.labels.length > 0 && (
        <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
          {issue.labels.map((label) => (
            <span
              key={label.id}
              className="flex items-center gap-1 rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-slate-300"
            >
...
function PlaceholderState() {
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-6 text-sm text-slate-400">
      Enter a <strong className="text-slate-200">owner/repo</strong> and choose a state to load issues.
    </div>
  );
}
...
function EmptyState({ owner, repo, state }: { owner: string; repo: string; state: string }) {
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-6 text-sm text-slate-400">
      No {state} issues found for <span className="font-semibold text-slate-200">{owner}/{repo}</span>.
    </div>
  );
}
